"""Tauri v2 Desktop Framework Generator for polyglot-framework-exporter.

Generates complete, runnable, production-ready Tauri v2 desktop application
bundling a Rust backend shell with modern React 19 / TypeScript / Tailwind CSS frontend
and Master Theme desktop integration.
"""
...
class TauriGenerator:
    """Generates a complete Tauri v2 desktop project with Master Theme integration."""

    name = "tauri"
    display_name = "Tauri v2 (Desktop)"
    description = "Lightweight, cross-platform native desktop application shell powered by Rust and Tauri v2."
    ...
    def generate(
        ...
        theme = ast.get_theme()
        files: Dict[str, str] = {}
        crate_name = ast.title.lower().replace(" ", "_").replace("-", "_")
        ...
        files["src-tauri/Cargo.toml"] = f"""[package]
name = "{crate_name}"
version = "{ast.version}"
description = "{html.escape(ast.description)}"
authors = ["{html.escape(ast.author)}"]
edition = "2021"
...
"""
        ...
        files["src-tauri/tauri.conf.json"] = json.dumps({
            "$schema": "https://raw.githubusercontent.com/tauri-apps/tauri/dev/crates/tauri-cli/schema.json",
            "productName": ast.title,
            "version": ast.version,
            "identifier": f"com.polyglot.{crate_name}",
            "build": {
...
"""
        ...
        files["src-tauri/src/main.rs"] = f"""// Prevents additional console window on Windows in release, DO NOT REMOVE!!
...
"""
        ...
        files["src-tauri/src/lib.rs"] = """// Learn more about Tauri commands at https://tauri.app/develop/calling-rust/
#[tauri::command]
...
"""
        ...
        files["src/components/Navbar.tsx"] = f"""import React from 'react';
  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur-md bg-bg-primary/80 border-b border-border-color transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <a href="/" className="flex items-center gap-2 text-xl font-heading font-bold text-text-primary tracking-tight hover:opacity-90">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-text-secondary">
          {{links.map((link, idx) => (
            <a key={{idx}} href={{link.href}} className="hover:text-accent-primary transition-colors">
              {{link.label}}
            </a>
          ))}}
        </nav>
        ...
        <div className="flex items-center gap-3">
          {{cta && (
            <a href={{cta.href}} className="px-4 py-2 rounded-lg text-sm font-semibold bg-accent-primary text-bg-primary hover:opacity-90 transition-all shadow-sm">
              {{cta.label}}
            </a>
          )}}
        </div>
...
"""
        ...
        files["src/components/Hero.tsx"] = f"""import React from 'react';
...
export default function Hero() {{
  ...
  const badge = "{h_data.get('badge', 'Tauri v2 Native Desktop Shell')}";
  ...
  const codeSnippet = "{h_data.get('code_snippet', 'cargo tauri dev')}";
    ...
    <section className="relative pt-24 pb-20 overflow-hidden border-b border-border-color">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
          <a href={{pCta.href}} className="px-6 py-3 rounded-xl font-semibold bg-accent-primary text-bg-primary hover:opacity-95 shadow-lg shadow-accent-primary/20 transition-all transform hover:-translate-y-0.5">
            {{pCta.label}}
          </a>
          <a href={{sCta.href}} className="px-6 py-3 rounded-xl font-semibold bg-bg-surface text-text-primary border border-border-color hover:bg-bg-surface-hover transition-all">
            {{sCta.label}}
          </a>
        </div>
        ...
        {{codeSnippet && (
          <div className="mt-10 max-w-md mx-auto">
            <div className="p-3 rounded-xl bg-bg-secondary border border-border-color flex items-center justify-between text-xs font-mono text-text-secondary">
              ...
              <span className="px-2 py-1 bg-bg-surface rounded border border-border-subtle text-xs">Rust Shell</span>
            </div>
          </div>
        )}}
...
"""
        ...
        files["src/components/Features.tsx"] = f"""import React from 'react';
...
"""
        ...
        files["src/components/Pricing.tsx"] = f"""import React from 'react';
          ...
          {{tiers.map((tier, idx) => (
            <div key={{idx}} className={{`p-8 rounded-2xl bg-bg-surface border flex flex-col justify-between transition-all duration-300 relative ${{tier.is_popular ? 'border-accent-primary shadow-xl ring-1 ring-accent-primary' : 'border-border-color'}}`}}>
              ...
              <div className="mt-8 pt-6 border-t border-border-subtle">
                <a href={{tier.cta_href || '#'}} className={{`block w-full text-center py-3 rounded-xl font-semibold transition-all ${{tier.is_popular ? 'bg-accent-primary text-bg-primary hover:opacity-90' : 'bg-bg-secondary text-text-primary hover:bg-bg-surface-hover border border-border-color'}}`}}>
...
"""
        ...
        files["src/components/Stats.tsx"] = f"""import React from 'react';
...
"""
        ...
        files["src/components/Testimonials.tsx"] = f"""import React from 'react';
...
"""
        ...
        files["src/components/FAQ.tsx"] = f"""import React, {{ useState }} from 'react';
...
"""
        ...
        files["src/components/CTA.tsx"] = f"""import React from 'react';
    ...
    <section className="py-20 relative overflow-hidden bg-accent-primary text-bg-primary">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        ...
        <div className="mt-8 flex justify-center gap-4">
          <a href={{pCta.href}} className="px-8 py-3.5 rounded-xl font-bold bg-bg-primary text-text-primary hover:opacity-90 shadow-xl transition-all">
            {{pCta.label}}
          </a>
        </div>
      </div>
...
"""
        ...
        files["src/components/CodeViewer.tsx"] = f"""import React, {{ useState }} from 'react';
...
"""
        ...
        files["src/components/Footer.tsx"] = f"""import React from 'react';
            ...
            <div key={{idx}}>
              ...
              <ul className="space-y-2 text-xs text-text-secondary">
                {{col.links.map((link, lIdx) => (
                  <li key={{lIdx}}>
                    <a href={{link.href}} className="hover:text-accent-primary transition-colors">
                      {{link.label}}
                    </a>
                  </li>
                ))}}
...
"""
        ...
        # 15. README.md
        files["README.md"] = f"""# {ast.title} (Tauri v2 Desktop Shell)
...
# Build release desktop binary (macOS .dmg/.app, Linux .deb/.AppImage, Windows .msi/.exe)
npm run tauri:build
```
"""
